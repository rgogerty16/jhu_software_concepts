...
def run_queries(database_url: str | None = None) -> dict:
    ...
    :param database_url: Optional Postgres connection string. When None, db.py
                         reads DATABASE_URL from the environment. Tests pass in
                         the URL of a throwaway database so production data is
                         never touched.
    ...
    with get_conn(database_url) as conn:
        with conn.cursor() as cur:
            ...
            # ------------------------------------------------------------------
            # Total row count — used in the page intro ("Analysis of X entries")
            # ------------------------------------------------------------------
